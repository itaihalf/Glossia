...
-- Creates a minimal user_profiles row immediately after a new auth user is
-- created. SECURITY DEFINER bypasses RLS so the auth schema context can
-- write to user_profiles. Onboarding upserts the full profile later;
...
-- Auto-create a user_profiles row on every new auth.users insert.
-- DROP + CREATE because CREATE OR REPLACE is not supported for triggers.
